...
#define RV_JTAG_TCK_PUT(tck) \
    if (tck) { \
        GPIO_BOP(RV_LINK_TCK_PORT) = RV_LINK_TCK_PIN; \
    } else { \
        GPIO_BC(RV_LINK_TCK_PORT) = RV_LINK_TCK_PIN; \
    }

#define RV_JTAG_TMS_MODE(out, tms) \
    if (out) { \
        GPIO_CTL1(RV_LINK_TMS_PORT) = 0x33380000; \
    } else { \
        GPIO_CTL1(RV_LINK_TMS_PORT) = 0x33880000; \
        if (tms) { \
            GPIO_BOP(RV_LINK_TMS_PORT) = RV_LINK_TMS_PIN; \
        } else { \
...
#define RV_JTAG_TDI_PUT(tdi) \
    if (tdi) { \
        GPIO_BOP(RV_LINK_TDI_PORT) = RV_LINK_TDI_PIN; \
    } else { \
        GPIO_BC(RV_LINK_TDI_PORT) = RV_LINK_TDI_PIN; \
    }
