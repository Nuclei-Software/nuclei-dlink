...
#include <rv-link/gdb-server/details/gdb-server-config.h>
#include <rv-link/gdb-server/details/gdb-server.h>
...
PT_THREAD(gdb_server_poll(void))
{
    ...
    RVL_DEBUG_LOG(&self.pt_server, (rvl_link_get_name()));
...
PT_THREAD(gdb_server_connected(void))
{
    ...
    gdb_resp_buf_puts("RV-LINK " RVL_GDB_SERVER_CONFIG_VERSION ": ");
    gdb_resp_buf_puts(rvl_link_get_name());
    gdb_resp_buf_puts(", configed for ");
    gdb_resp_buf_puts(rvl_target_get_name());
    gdb_resp_buf_puts(" family.\n");
